...
export class ActionComponent implements OnInit {
  ...
  onActionClick(actionData: IAction) {
    if (actionData.type === 2 || actionData.type === 5) {
      ...
      this.index.emit(false);
    }
    this.selectedIndex = actionData.id;
  ...
  postNewAction() {
    this.actionService.postAction().subscribe(res => {
      ...
      if (this.actionService.action.type === 3 || this.actionService.action.type.toString() === '3') {
  ...
  editChildAction(form: NgForm) {
    this.childActionService.updateChildAction().subscribe(res => {
      ...
      const updateChildActionIndex = this.childAction.findIndex(z => z.id ===
        this.childActionService.childAction.id);
